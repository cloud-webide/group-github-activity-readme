...
const _getIssues = async (params) => {
    ...
    const issues = await exports.octokit.paginate(exports.octokit.rest.issues.listForRepo, {
        owner,
        repo,
        page,
        per_page,
...
exports["default"] = (params) => {
    return _getIssues(params).then((data) => {
        const issues = data.filter(({ html_url }) => html_url.includes('issues'));
        const pullRequests = data.filter(({ html_url }) => html_url.includes('pull'));
        return {
            issues,
            pullRequests,
...
const issue_1 = __importDefault(__nccwpck_require__(5075));
// Get config
...
const run = async () => {
    ...
    for (const user of users) {
        for (const namespace of namespaces) {
            ...
            const { issues = [], pullRequests = [], repository_name, } = await (0, issue_1.default)({
                owner,
                repo,
                creator: user,
    ...
    if (!newContents.length) {
        ...
        // );
        console.info('No PullRequest/Issue/IssueComment/Release events found. Leaving README unchanged with previous activity');
        return;
    }
